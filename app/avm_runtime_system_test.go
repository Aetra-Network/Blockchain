...
func TestAVMRuntimeAppLevelDeployExecuteQueueStorageReceiptsAndExportImport(t *testing.T) {
	...
	params := app.ContractsKeeper.Params()
	params.Authority = account.AddressUser
	require.NoError(t, app.ContractsKeeper.UpdateContractParams(contractstypes.MsgUpdateContractParams{
		Authority: app.ContractsKeeper.Params().Authority,
		Params:    params,
	}))
	...
	_, err = app.ContractsKeeper.ReceiveInternalMessage(contractstypes.MsgReceiveInternalMessage{
		SourceContractUser: deployed.ContractAddressUser,
		DestinationAccount: destination,
		Funds:              7,
		Opcode:             1,
...
func TestAVMRuntimeRejectsFrozenAccountsReservedZeroAndRecoversFrozenContracts(t *testing.T) {
	...
	codeID := appAVMRuntimeStoreCode(t, app, ctx, app.ContractsKeeper.Params().Authority)
...
func TestAVMRuntimeDeterminismGateRejectsNondeterminismAndKeepsStableRoots(t *testing.T) {
	...
	account := nativeAccountActivateViaRoute(t, app, ctx, nativeAccountModuleTestPubKey())
	appParams := app.ContractsKeeper.Params()
	appParams.Authority = account.AddressUser
	require.NoError(t, app.ContractsKeeper.UpdateContractParams(contractstypes.MsgUpdateContractParams{
		Authority: app.ContractsKeeper.Params().Authority,
		Params:    appParams,
	}))
