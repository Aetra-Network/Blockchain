...
func TestClaimPoolRewardsRejectsOwnerSignerMismatch(t *testing.T) {
	k := NewKeeper()
	pool := createPool(t, &k, "pool-mismatch")
	owner := rawPoolAddress("22")
	ownerAE := aePoolAddress(t, "22")
	other := aePoolAddress(t, "33")
	deposit(t, &k, pool.PoolID, owner, 1_000, 2)
	_, err := k.ApplyPoolReward(pool.PoolID, 400)
	require.NoError(t, err)

	_, err = k.ClaimPoolRewards(types.MsgClaimPoolRewards{
		Authority:    other,
		PoolID:       pool.PoolID,
		OwnerAddress: ownerAE,
		Height:       4,
	})
	require.ErrorContains(t, err, "signer must match owner address")
}
