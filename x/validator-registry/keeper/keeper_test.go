...
type recordingValidatorRegistryStoreService struct {
	store   *kvtest.Store
	lastCtx context.Context
}

func newRecordingValidatorRegistryStoreService() *recordingValidatorRegistryStoreService {
	return &recordingValidatorRegistryStoreService{store: kvtest.NewStoreService().RawStore()}
}

func (s *recordingValidatorRegistryStoreService) OpenKVStore(ctx context.Context) corestore.KVStore {
	s.lastCtx = ctx
	return s.store
}
...
func TestMsgServerUsesCurrentContextForPersistence(t *testing.T) {
	...
	service := newRecordingValidatorRegistryStoreService()
	source := NewPersistentKeeper(service)
	gs := DefaultGenesis()
	gs.State.Validators = []types.ValidatorRecord{testValidator(0x51, "ed25519:persistent-context")}
	require.NoError(t, source.InitGenesisState(initCtx, gs))

	srv := NewMsgServerImpl(&source)
	_, err := srv.UpdateValidatorMetadata(txCtx, &v1.MsgUpdateValidatorMetadata{
		Authority:       prototype.DefaultAuthority,
		OperatorAddress: gs.State.Validators[0].OperatorAddress,
		Metadata:        `{"moniker":"updated"}`,
		Height:          2,
	})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	restored, found := exported.State.Validator(gs.State.Validators[0].OperatorAddress)
	require.True(t, found)
	require.Equal(t, `{"moniker":"updated"}`, restored.Metadata)
}
