...
func (v ValidatorRecord) Validate(params Params) error {
	...
	if err := ValidateConsensusPublicKey("validator consensus public key", v.ConsensusPublicKey, params.MaxConsensusKeyBytes); err != nil {
		return err
	}
	if strings.TrimSpace(v.PendingConsensusPublicKey) != "" {
		if err := ValidateConsensusPublicKey("validator pending consensus public key", v.PendingConsensusPublicKey, params.MaxConsensusKeyBytes); err != nil {
			return err
		}
...
func (v ValidatorRecord) Normalize(params Params) ValidatorRecord {
	...
	if normalized, err := NormalizeConsensusPublicKey(v.ConsensusPublicKey); err == nil {
		v.ConsensusPublicKey = normalized
	}
	if strings.TrimSpace(v.PendingConsensusPublicKey) != "" {
		if normalized, err := NormalizeConsensusPublicKey(v.PendingConsensusPublicKey); err == nil {
			v.PendingConsensusPublicKey = normalized
		}
	}
...
func validateConsensusKey(label, value string, maxBytes uint32) error {
	return ValidateConsensusPublicKey(label, value, maxBytes)
}
