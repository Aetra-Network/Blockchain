...
func TestPersistentMsgServerMutationSurvivesRestart(t *testing.T) {
	ctx := sdk.WrapSDKContext(sdk.Context{}.WithBlockHeight(1).WithEventManager(sdk.NewEventManager()))
	...
	msgServer := NewMsgServerImpl(&source)
	_, err := msgServer.CreateSingleNominatorPool(ctx, &v1.MsgCreateSingleNominatorPool{
		Authority:       prototype.DefaultAuthority,
	...
	require.NoError(t, err)
	_, err = msgServer.DepositSingleNominator(ctx, &v1.MsgDepositSingleNominator{
		Authority:   prototype.DefaultAuthority,
		PoolAddress: rawSingleAddress("10"),
		Owner:       rawSingleAddress("11"),
		Amount:      1_000,
	...
	require.Equal(t, uint64(1_000), exported.State.Pools[0].BondedStake)
	require.Equal(t, rawSingleAddress("12"), exported.State.Pools[0].Validator)
}
