...
func TestExportImportPreservesLiveInternalMessageFlow(t *testing.T) {
	executor := newTestExecutor(t)
	deployer := testAddr(1)
	source := deployTestContract(t, executor, deployer, []byte("live-source"))
	destination := deployTestContract(t, executor, deployer, []byte("live-destination"))

	require.NoError(t, executor.RegisterHandler(source, func(contract ContractAccount, msg MessageEnvelope) ExecutionResult {
		return ExecutionResult{
			NewState: []byte("source:sent"),
			Outgoing: []MessageEnvelope{{
	...
	require.NoError(t, executor.RegisterHandler(destination, func(contract ContractAccount, msg MessageEnvelope) ExecutionResult {
		return ExecutionResult{
			NewState:   append([]byte("dest:"), msg.Body...),
			ResultCode: ResultOK,
		}
	}))

	require.NoError(t, executor.EnqueueTxMessages([]MessageEnvelope{testMessage(testAddr(9), source, 9)}))
	receipts, err := executor.ProcessBlock(1)
	require.NoError(t, err)
	require.Len(t, receipts, 2)
	...
	sourceContract, ok := imported.Contract(source)
	require.True(t, ok)
	require.Equal(t, []byte("source:sent"), sourceContract.State)
	destinationContract, ok := imported.Contract(destination)
	require.True(t, ok)
	require.Equal(t, []byte("dest:live-body"), destinationContract.State)
}
