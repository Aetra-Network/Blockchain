...
type MessageEnvelope struct {
	...
	// Mode is the combined send-mode flag bitmask (SEND_* constants OR'd
	// together, see compiler builtinSendModeValue). It selects value/error
	// semantics at delivery: SEND_DRAIN_BALANCE (128) sends the source's full
	// balance, SEND_DESTROY_IF_EMPTY (32) deletes the source once it is empty,
	// SEND_IGNORE_ERRORS (2) swallows delivery failure of THIS message. Zero
	// is SEND_DEFAULT.
	Mode uint32
	// Comment is the optional user-facing text memo attached to the message
	// (the language's textComment). At most one per message; carried through
	// to the receipt/queue so an explorer can display "who sent what to whom
	// with what note". Bounded by MaxCommentBytes.
	Comment string
}

// Send-mode flag bits. These mirror the compiler's builtinSendModeValue so the
// VM and keeper agree on the meaning of a MessageEnvelope.Mode bitmask.
const (
	SendModeFeeFromBalance uint32 = 1
	SendModeIgnoreErrors   uint32 = 2
	SendModeBounceOnFail   uint32 = 16
	SendModeDestroyIfEmpty uint32 = 32
...
// through the normal message body/byte fee, so a large comment simply costs
// more; this cap keeps a single memo from bloating state or a block.
const MaxCommentBytes = 512

// HasMode reports whether the given flag bit is set in the mode bitmask.
func HasMode(mode, flag uint32) bool { return mode&flag != 0 }
