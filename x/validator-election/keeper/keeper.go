...
func (k *Keeper) EndBlocker(ctx sdk.Context) error {
	...
	window := k.genesis.State.ElectionWindow
	if window.WithdrawDeadlineHeight > 0 && height >= window.WithdrawDeadlineHeight {
		...
		if !hasUncommitted {
			...
			if pendingCount > 0 {
				if _, err := k.CommitElection(types.MsgCommitElection{
	...
	if window.EndHeight > 0 && height >= window.EndHeight {
		hasCommitted := false
		for _, result := range k.genesis.State.ElectionResults {
			if result.Epoch == k.genesis.State.ElectionEpoch && result.Committed {
				hasCommitted = true
		...
		if hasCommitted {
			hasFinalized := false
			for _, result := range k.genesis.State.ElectionResults {
				if result.Epoch == k.genesis.State.ElectionEpoch && result.Finalized {
					hasFinalized = true
					break
				}
			}
			if !hasFinalized {
				if _, err := k.FinalizeElection(types.MsgFinalizeElection{
					Authority: k.genesis.Params.Authority,
					Height:    height,
				}); err != nil {
					return err
