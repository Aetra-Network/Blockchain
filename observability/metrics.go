...
var Definitions = []Definition{
	...
	{MetricEconomyBurnedFeesNaet, "Cumulative protocol coins burned in naet (fee routing plus the emission burn bucket).", kindGauge},
	...
	{MetricSlashingEventsTotal, "Slashing events observed from validator state transitions, with bounded reason labels (downtime, double_sign).", kindCounter},
	{MetricValidatorJailEventsTotal, "Validator jail transitions observed at sweep granularity, with bounded reason labels.", kindCounter},
	{MetricValidatorUnjailEventsTotal, "Validator unjail transitions observed at sweep granularity.", kindCounter},
	...
	{MetricValidatorMissedBlocks, "Missed blocks aggregated across the validator set (positive signing-info counter deltas between observations).", kindCounter},
	{MetricValidatorUptimeBps, "Bonded validator uptime in basis points over the slashing signed-blocks window, labeled stat=min|avg.", kindGauge},
