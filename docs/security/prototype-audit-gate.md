...
| `GO-2024-2584` in `github.com/cosmos/cosmos-sdk@v0.54.3` (GHSA-86h5-xcpx-cfqc / ASA-2024-005, slashing evasion during re-delegation) | `govulncheck -scan=package` | Resolved (false positive) | Dedicated review complete: cosmos-sdk's own advisory lists fixed versions `v0.47.10`/`v0.50.5`; this repo's pinned `v0.54.3` is well past both, and the fix (`x/staking/keeper/slash.go` unbonding-delegation-priority handling) is present in the vendored source. `govulncheck` only still flags it because the upstream OSV range for the 0.50+ branch has no closing `fixed` event -- a database gap, not a live issue. See `docs/security/security-gates-triage.md` for the full writeup. |
...
| Vote extension handler readiness | `app/abcihandlers/vote_extension.go` determinism scan | Resolved | `crypto/rand` dummy bytes were replaced with `DeterministicVoteExtensionData` (tamper-rejecting, size- and kind-bounded; see `app/abcihandlers/vote_extension_test.go`). The handler is also not wired into app construction (`SetHandlers` is never called) and genesis never sets `Abci.VoteExtensionsEnableHeight`, so CometBFT never invokes the pipeline; `TestVoteExtensionsAreDisabledAtConsensusParamsLevel` in `app` pins that gate. Re-audit before wiring `SetHandlers` for a real consumer. |
