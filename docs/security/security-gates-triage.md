...
| `GO-2024-2584` slashing evasion in `github.com/cosmos/cosmos-sdk@v0.54.3`, reached via a direct, non-spurious call path: `app.L1App.BeginBlocker` -> `module.Manager.BeginBlock` -> `staking keeper.Keeper.SlashWithInfractionReason` (runs every block as part of normal consensus operation) | Security + Consensus | High | consensus-safety dependency | accepted (needs dedicated review) | No fixed `cosmos-sdk` release exists yet for this advisory. This is the most serious of the three accepted findings because the vulnerable code path is unconditionally reachable in consensus (not gated behind an optional feature). Requires a dedicated security-team review of the exact evasion mechanism against this chain's slashing configuration before public testnet, and an upstream `cosmos-sdk` upgrade as soon as one is available. Not to be waved through as routine dependency risk. | public-testnet-gate |
