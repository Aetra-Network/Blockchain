...
| Unauthorized contract-assets | Contract-asset mint, transfer, burn, or metadata change is rejected with an authorization error. | `build\aetrad.exe query contract-assets denom $DENOM --grpc-addr $GRPC --grpc-insecure --node $NODE --output json`; inspect the failing contract tx and events. | Use the current owner/admin path for the asset standard being tested. Confirm the rejection is correct and add or update the regression test if the signer should have been authorized. |
